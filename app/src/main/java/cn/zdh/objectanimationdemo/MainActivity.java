...
//        rotationAnimator.setDuration(200);//动画必须设置时间；
        ...
        scaleXAnimator.setDuration(200);//动画必须设置时间；

        ObjectAnimator scaleYAnimator = ObjectAnimator.ofFloat(iv1, "scaleY", 0.8f, 1f);
        scaleYAnimator.setDuration(200);//动画必须设置时间；
...
//        UnRotationAnimator.setStartDelay(200);//延迟执行
        ...
        //平移动画 (动画控件；        什么动画：动画名称 比如平移动画为translation，rotationY是X还是Y主要看动画以X为中心翻转还是Y；         翻转角度:从多到多少 顺时针为正逆时针为负）
        ObjectAnimator translationYAnimator = ObjectAnimator.ofFloat(iv1, "translationY", -iv1.getHeight() * 0.1f, 0f);//因为缩放到0.8，剩下的0.2上下个占一半所以上移空间为0.1f
        translationYAnimator.setDuration(300);//动画必须设置时间；
